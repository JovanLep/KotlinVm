...
class XnChatUtil private constructor() {
    ...
    fun initXNLibrary() {
        ...
        if (0 == flag) {
            ...
            Log.e("TAG","XN 初始化SDK失败，退出登录，重新初始化，错误码:$flag")
            xnLogout()
            ...
            if (0 == initXNSDK) {
                ...
                Log.e("TAG","XN 初始化SDK失败，错误码:$initXNSDK")
            }
        }
    }
    ...
    private fun initXNLibraryOtherParams() {
        Log.e("TAG","XN 初始化SDK成功")
        xnLogin()
        val enableDebug = Ntalker.getBaseInstance().enableDebug(BuildConfig.DEBUG) // 选择debug模式
        if (0 == enableDebug) {
            Log.e("TAG","XN enableDebug 执行成功")
        } else {
            Log.e("TAG","XN enableDebug 执行失败，错误码:$enableDebug")
        }
...
//            context!!.resources.getString(R.string.detail_service_name) //咨询发起页标题(必填)
     ...
     * @param fragmentActivity 上下文环境
     */
//    fun startProductDetialChat2(fragmentActivity: FragmentActivity, productDetail: ProductDetail) {
//        val chatparams = ChatParamsBody()
//        // 咨询发起页（专有参数）
